                    ...
                    for algorithm in algorithm_range:
                        for wireless_technology in wireless_technology_range:
                            ...
                            if (num_depots == 1 and (algorithm == 0 or algorithm == 1)) or (num_depots > 1 and (algorithm == 2 or algorithm == 3)):
                                ...
                                fullCommand = f"{baseCommand} -exp_name {expName} -iterations {iterations} -num_sensors {num_sensors} -num_depots {num_depots} -sensor_radius {sensor_radius} -energy_budget {energy_budget} -wireless_technology {wireless_technology} -algorithm {algorithm} -seed {it} -scenario {scenario}"
